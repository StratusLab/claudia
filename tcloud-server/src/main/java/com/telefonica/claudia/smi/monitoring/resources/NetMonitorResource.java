...
public class NetMonitorResource extends MonitorResource {
	
	private static Logger log = Logger.getLogger(NetMonitorResource.class);
	
	public NetMonitorResource(Context context, Request request, Response response) {
        super(context, request, response);
        log.info("NetMonitorResource created");//log
        
        //this.setUpType(TYPE_NET);
	...
	public Representation represent(Variant variant) throws ResourceException {
		...
		if (MediaType.TEXT_XML.equals(variant.getMediaType())) {
			log.info("NetMonitorResource.represent");// log
			...
			try {
				mdl = actualDriver.getNetMeasureDescriptorList(orgId, vdcId, netId);
			} catch (MonitorException e) {
				log.debug(e.getMessage());
				return getUnknownElementErrorRepresentation(e.getMessage());
			}

			setMeasureDescriptorListHrefsLinks(mdl, TYPE_NET);
			log.info("Data returned for monitor "
					+ URICreation.getNetworkFQN(orgId, vdcId, vappIds.get(0), netId)
					+ ": \n\n" + mdl);
