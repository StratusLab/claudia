...
public class VDCItemCollectionResource extends Resource  {
	
	private static Logger log = Logger.getLogger("com.telefonica.claudia.smi.VDCItemCollectionResource");

    String orgId;
    
    
    public VDCItemCollectionResource(Context context, Request request, Response response) {
    	...
    	if (entity.getMediaType().equals(MediaType.APPLICATION_XML)) {
    		...
    		try {
				...
				try {
					...
					log.error("Error transforming XML content. The XML document may be malformed: " + e.getMessage());
				...
				taskId = actualDriver.createVdc(URICreation.getFQN(orgId), sw.toString());
			} catch (IOException e) {
			...
			// Send the response 
			try {
				String task = TaskManager.getInstance().getTask(taskId).getStringDescription().replace("{org-id}", orgId);
				StringRepresentation representation = new StringRepresentation(task, MediaType.TEXT_XML);
