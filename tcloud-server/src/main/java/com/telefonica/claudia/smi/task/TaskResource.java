...
public class TaskResource extends Resource {
    ...
    public TaskResource(Context context, Request request,  
        ...
        if (vdcId != null && orgId!=null && taskId!=null) {
    ...
    public Representation represent(Variant variant) throws ResourceException {
    	...
    	if (MediaType.TEXT_XML.equals(variant.getMediaType())) {
			...
			try {
				String taskDefinition = TaskManager.getInstance().getTask(Long.parseLong(taskId)).getStringDescription().replace("{org-id}", orgId).replace("{vdc-id}", vdcId);
